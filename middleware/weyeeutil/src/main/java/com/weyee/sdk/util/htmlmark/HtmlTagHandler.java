 ...
 */
final class HtmlTagHandler implements Html.TagHandler {
     ...
     * @see
     * <a href="https://github.com/android/platform_frameworks_base/commit/8b36c0bbd1503c61c111feac939193c47f812190">Specific Android SDK Commit</a>
     */
    ...
    @Override
    public void handleTag(final boolean opening, final String tag, Editable output, final
    XMLReader xmlReader) {
        if (opening) {
            // opening tag
            ...
            if (tag.equalsIgnoreCase(UNORDERED_LIST)) {
                ...
                if (!lists.isEmpty()) {
                    if (lists.peek().equalsIgnoreCase(UNORDERED_LIST)) {
                        ...
                        // Nested BulletSpans increases distance between bullet and text, so we
                        // must prevent it.
                        int bulletMargin = indent;
                        if (lists.size() > 1) {
                            bulletMargin = indent - bullet.getLeadingMargin(true);
                            if (lists.size() > 2) {
                                // This get's more complicated when we add a LeadingMarginSpan
                                // into the same line:
                ...
                end(output, Center.class, true, new AlignmentSpan.Standard(Layout.Alignment
                        .ALIGN_CENTER));
            } else if (tag.equalsIgnoreCase("s") || tag.equalsIgnoreCase("strike")) {
    ...
    private void endFont(Editable output) {
        ...
        if (where != len) {
            ...
            if (color != -1) {
                output.setSpan(new ForegroundColorSpan(color | 0xFF000000), where, len, Spanned
                        .SPAN_EXCLUSIVE_EXCLUSIVE);
            }
            if (size > 0) {
                output.setSpan(new AbsoluteSizeSpan(size, true), where, len, Spanned
                        .SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
