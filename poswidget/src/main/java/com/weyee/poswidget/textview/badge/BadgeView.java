...
package com.weyee.poswidget.textview.badge;
...
public class BadgeView extends View implements Badge {
    ...
    private void init() {
        ...
        mBadgeTextSize = SizeUtils.sp2px( DEFAULT_TEXT_SIZE);
        mBadgePadding = SizeUtils.dp2px(DEFAULT_BADGE_PADDING);
        mBadgeNumber = 0;
        mBadgeGravity = Gravity.END | Gravity.TOP;
        mGravityOffsetX = SizeUtils.dp2px(DEFAULT_GRAVITY_OFFSET);
        mGravityOffsetY = SizeUtils.dp2px(DEFAULT_GRAVITY_OFFSET);
        mFinalDragDistance = SizeUtils.dp2px( 90);
        mShowShadow = true;
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                if (mDraggable && event.getPointerId(event.getActionIndex()) == 0
                    ...
                    mDefaultRadius = SizeUtils.dp2px( 7);
    ...
    protected Bitmap createBadgeBitmap() {
        Bitmap bitmap = Bitmap.createBitmap((int) mBadgeBackgroundRect.width() + SizeUtils.dp2px( 3),
                (int) mBadgeBackgroundRect.height() + SizeUtils.dp2px( 3), Bitmap.Config.ARGB_8888);
    ...
    private void showShadowImpl(boolean showShadow) {
        int x = SizeUtils.dp2px( 1);
        int y = SizeUtils.dp2px( 1.5f);
        switch (mDragQuadrant) {
            case 1:
                x = SizeUtils.dp2px( 1);
                y = SizeUtils.dp2px( -1.5f);
                break;
            case 2:
                x = SizeUtils.dp2px( -1);
                y = SizeUtils.dp2px( -1.5f);
        ...
        mBadgeBackgroundPaint.setShadowLayer(showShadow ? SizeUtils.dp2px( 2f)
                : 0, x, y, 0x33000000);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBadgeText != null) {
            ...
            if (mDraggable && mDragging) {
                ...
                if (mDragOutOfRange = startCircleRadius < SizeUtils.dp2px( 1.5f)) {
    ...
    public Badge stroke(int color, float width, boolean isDpValue) {
        ...
        mBackgroundBorderWidth = isDpValue ? SizeUtils.dp2px( width) : width;
        invalidate();
        return this;
    }
    ...
    public Badge setBadgeTextSize(float size, boolean isSpValue) {
        mBadgeTextSize = isSpValue ? SizeUtils.dp2px( size) : size;
        measureText();
        invalidate();
        return this;
    }
    ...
    public float getBadgeTextSize(boolean isSpValue) {
        return isSpValue ? SizeUtils.px2dp( mBadgeTextSize) : mBadgeTextSize;
    }

    @Override
    public Badge setBadgePadding(float padding, boolean isDpValue) {
        mBadgePadding = isDpValue ? SizeUtils.dp2px( padding) : padding;
        createClipLayer();
        invalidate();
        return this;
    }
    ...
    public float getBadgePadding(boolean isDpValue) {
        return isDpValue ? SizeUtils.px2dp( mBadgePadding) : mBadgePadding;
    }
    ...
    public Badge setGravityOffset(float offsetX, float offsetY, boolean isDpValue) {
        mGravityOffsetX = isDpValue ? SizeUtils.dp2px( offsetX) : offsetX;
        mGravityOffsetY = isDpValue ? SizeUtils.dp2px( offsetY) : offsetY;
        invalidate();
        return this;
    }
    ...
    public float getGravityOffsetX(boolean isDpValue) {
        return isDpValue ? SizeUtils.px2dp( mGravityOffsetX) : mGravityOffsetX;
    }

    @Override
    public float getGravityOffsetY(boolean isDpValue) {
        return isDpValue ? SizeUtils.px2dp( mGravityOffsetY) : mGravityOffsetY;
    }
